...
seq_length = 15
model = load_model(
    'C:/Users/UCL7/VS_kwix/created_dataset/dataset_version9/side_lunge/model_v9_3.h5')
pose = set_cam()
N_scaler = Normalizer()

seq, action_seq = [], []
cap = cv2.VideoCapture(
    'C:/Users/UCL7/VS_kwix/Byungsun_main/create_model/side_lunge2.mp4')
...
while True:
    ...
    if results.pose_landmarks is not None:
        landmark_subset = draw_function.custom_landmarks(results)
        ...
        if action_seq[-1] == action_seq[-2]:
            if y_pred[0] > 0.8:
                rate = 0
                img = draw_function.drawing_Side_lunge(
    ...
    cv2.putText(img, 'Standing side lunge',  org=(
        0, 180), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=1, color=(255, 255, 255), thickness=2)

    cv2.rectangle(img, (0, 260), (int(y_pred[1]*350), 320), (117, 245, 16), -1)
