...
os.makedirs('C:/Users/UCL7/Desktop/Kwix_HAR/test_dataset_v1/', exist_ok=True)

for i, _ in enumerate(actions):
    os.makedirs('C:/Users/UCL7/Desktop/Kwix_HAR/test_dataset_v1/' + _actions[i], exist_ok=True)
    ...
    for idx, action in enumerate(_):
        ...
        for frame in img_file:
            ...
            if results.pose_landmarks is not None:
                ...
                if idx == 0:
                    label = np.array([0])   # 불량한 자세
                else: 
                    label = np.array([1])   # 올바른 자세
                d = np.concatenate([joint.flatten(), label])
                data.append(d)

        data = np.array(data)
        print(action, data.shape)
        np.save(os.path.join('C:/Users/UCL7/Desktop/Kwix_HAR/test_dataset_v1/' + _actions[i],
