...
def BiLS_CNN():
    model = Sequential()
    model.add(Bidirectional(LSTM(64, return_sequences=True, input_shape=(None, 15, 61), dropout=0.3)))
...
def CNN_BiLS():
    model = Sequential()
    model.add(Conv1D(filters=64, kernel_size=4, padding='valid'))
    model.add(Dropout(rate=0.5))
    model.add(BatchNormalization())
    model.add(ReLU())
    ...
    model.add(Bidirectional(LSTM(16, return_sequences=True, dropout=0.3)))
